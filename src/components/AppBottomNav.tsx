...
import EventFeedWithTopNav from "./EventFeedWithTopNav";
      ...
      <Tab.Navigator
        screenOptions={({ route }) => ({
          ...
          tabBarIcon: ({ focused, color = 'white', size }) => {
            ...
            switch (route.name) {
              case 'Events':
                return <EventsIcon color={color} width={size} height={size} focused={focused} />;
              case 'Discover':
                iconName = 'search';
                break;
              case 'Notifications':
      ...
      >
        <Tab.Screen
          options={{
            ...
            headerTitle: (props) => <View {...props} style={{ flex: 1, justifyContent: 'center', alignItems: 'center', flexDirection: 'row', gap: spacing.sm }}   >
             <Icon name="location-outline" size={20} color={theme.colors.primary} />  <Text style={{ color:'white' }}>Jubilee Hills, Hyderabad</Text> 
             <Icon name="chevron-down-outline" size={20} color={'white'} />
            </View>
          }}
          name="Events"
          component={EventFeedWithTopNav} />
